...
public class QueryTests {
	...
	public void testSingle() {
		...
		StepVerifier.create(singleEmployee).expectNextMatches(employee -> {
	...
	public void testMultiple() {
		...
		ExampleMatcher matcher = ExampleMatcher.matching().withIgnoreCase().withMatcher("lastName", startsWith())
		...
		StepVerifier.create(multipleEmployees.collectList()).expectNextMatches(employees -> {
			assertThat(employees).hasSize(2);
			assertThat(employees).extracting("firstName").contains("Frodo", "Bilbo");
			return true;
		}).expectComplete().verify();
	}
